...
// WorktreeRemover removes a git worktree. The dir parameter is the
// working directory from which the git command runs.
type WorktreeRemover func(worktreeDir, dir string) error
...
// NewGitTagResolver creates a GitTagResolver with the given dependencies.
func NewGitTagResolver(tag string, deps Deps) *GitTagResolver {
	return &GitTagResolver{Tag: tag, deps: deps}
}

// Resolve returns the path to the binary for the named utility, building
// from the git tag if necessary.
...
// ListUtilities returns the names of all binaries built from the tag.
...
func (r *GitTagResolver) ensureBuild() error {
	...
	cmd := exec.Command(r.deps.GitBin, "worktree", "add", wtDir, r.Tag)
	...
	for _, e := range entries {
		...
		r.deps.Log("compare: building %s from %s", name, r.Tag)
		build := exec.Command(r.deps.GoBin, "build", "-o", outPath, pkgPath)
		build.Dir = wtDir
...
func (r *GitTagResolver) cleanup() {
	if r.wtDir != "" {
		if err := r.deps.RemoveWorktree(r.wtDir, "."); err != nil {
			r.deps.Log("compare: warning: removing worktree %s: %v", r.wtDir, err)
		}
		r.wtDir = ""
	}
	if r.buildDir != "" {
		if err := os.RemoveAll(r.buildDir); err != nil {
			r.deps.Log("compare: warning: removing build dir %s: %v", r.buildDir, err)
		}
		r.buildDir = ""
	}
}
...
// BinaryName returns the system binary name for a utility. Coreutils
// get a "g" prefix; moreutils keep their original name.
...
// Resolve looks up the binary on the system PATH.
...
// ListUtilities discovers available GNU coreutils and moreutils binaries.
...
// Resolve returns the path to the named utility in the directory.
...
// ListUtilities returns the names of all files in the directory.
...
func ResolverFromArg(arg string, deps Deps) BinaryResolver {
	...
	return NewGitTagResolver(arg, deps)
}
...
// Run executes differential comparison between two binary sources.
...
func Run(argA, argB, utility string, deps Deps) error {
	resolverA := ResolverFromArg(argA, deps)
	resolverB := ResolverFromArg(argB, deps)

	cases, err := LoadCompareTestCases(DefaultSpecsDir)
	if err != nil {
	...
	deps.Log("compare: %d utilities to compare between %s and %s", len(utilities), argA, argB)
	...
	for _, util := range utilities {
		...
		if len(utilCases) == 0 {
			deps.Log("compare: skipping %s (no test cases)", util)
			continue
		}

		pathA, cleanupA, err := resolverA.Resolve(util)
		if err != nil {
			deps.Log("compare: skipping %s: resolver A: %v", util, err)
			continue
		}
		...
		if err != nil {
			deps.Log("compare: skipping %s: resolver B: %v", util, err)
			continue
		}
		cleanups = append(cleanups, cleanupB)

		deps.Log("compare: running %d test cases for %s", len(utilCases), util)
